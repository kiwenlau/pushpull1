...
echo -e "\nssh $username@$remoteip \"docker run --name=\"docker_registry\" -p 5000:5000 -v /etc/localtime:/etc/localtime:ro -v ~/registry:/tmp/registry -d kiwenlau/registry:$registryversion\""
ssh $username@$remoteip "docker rm -f docker_registry &>/dev/null"
ssh $username@$remoteip "docker run --name="docker_registry" -p 5000:5000 -v ~/registry:/tmp/registry -d kiwenlau/registry:$registryversion &>/dev/null"
ssh $username@$remoteip "docker ps | grep registry"
...
then
	...
	echo -e "\nssh $username@$remoteip \"docker rm -f docker_registry\""
	ssh $username@$remoteip "docker rm -f docker_registry &>/dev/null"
	docker ps | grep registry
	echo ""
	#delete data volume
        ssh $username@$remoteip "echo $password | sudo -S rm -rf registry &>/dev/null"
	echo ""
	exit 60
fi	
...
# save registry logs
mkdir logs &> /dev/null
ssh $username@$remoteip "docker logs -t docker_registry 2>&1" >> logs/$imagename.txt
echo -e "\n\n\n" >> logs/$imagename.txt
